...
class Admin extends Shop_Admin_Controller {
    ...
    function Admin(){
        ...
        // Set breadcrumb
        $this->bep_site->set_crumb("Lilly Fairies ".$this->lang->line('backendpro_pages'),$this->module.'/admin');
    }


    function index(){
        // we use the following variables in the view
        $data['title'] = $this->lang->line('kago_manage_page'). " of Lilly Fairies";
        ...
        $data['header'] = "Lilly Fairies ".$this->lang->line('kago_page');
    ...
    function create(){
        ...
        if ($this->input->post('name')){
            ...
            // Set breadcrumb
            $this->bep_site->set_crumb($this->lang->line('kago_create'),'admin/create');
    ...
    function edit($id=0){
        ...
        $pagecontent = $this->MPages->getPage($content_id,$this->table);
        ...
        if (!count($data['page'])){
            ...
            //$data['menus'] = $this->MMenus->getAllMenusDisplay();
            // Set breadcrumb
            $this->bep_site->set_crumb($this->lang->line('kago_edit'),'admin/');
             // if lang_id is not 0 then they are translation
        /*
        $lang_id = $pagecontent['lang_id'];
        if($lang_id>0){
            $this->bep_site->set_crumb($this->lang->line('kago_edit_translation'),'admin/edit/'.$content_id);
        }
         *
         */
        $data['header'] = $this->lang->line('backendpro_access_control');
 ...
 */
    function delete($id){
        ...
        if(!empty($checkmenu)){
            ...
            redirect('lilly_fairies_menus/admin/index','refresh');
        }else{
            // delete it
...
function langcreate(){
    ...
    if ($this->input->post('name')){
        ...
        $checktrans =$this->MKaimonokago->checktrans($this->table,$path, $lang_id);
        if (count($checktrans)){
       ...
       // flashMsg('warning',$this->lang->line('kago_translation_exists'));
        ...
        // get all the languages
        $data['languages'] =$this->MLangs->getLangDropDownWithId();
        $data['translanguages'] =$this->MLangs->getTransLang($this->table,$path);
        ...
        //$data['menus'] = $this->MMenus->getAllMenusDisplay();
        // Set breadcrumb
        $this->bep_site->set_crumb($this->lang->line('kago_edit'),'admin/edit/'.$id);
        //$this->bep_site->set_crumb($this->lang->line('kago_add_translation').$selected_lang,'admin/edit/'.$id."/".$lang_id);
        $data['header'] = $this->lang->line('backendpro_access_control');
