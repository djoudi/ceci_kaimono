...
class Admin extends Shop_Admin_Controller {
    ...
    function Admin(){
        ...
        // Set breadcrumb
        $this->bep_site->set_crumb(" Lilly Fairies ".$this->lang->line('backendpro_menus'),$this->module.'/admin');
    }
  

  function index(){
        $data['title'] = $this->lang->line('kago_manage'). " Lilly Fairies ". $this->lang->line('kago_menu');
        $tree = array();
        $parentid = 0;
        //$this->MMenus->generateallTreewithUriId($tree,$parentid);
        $lilly_menutable = $this->table;
        $this->MMenus->generateallTree($tree,$parentid,$lilly_menutable);
        $data['navlist'] = $tree;
        // get pages which is used to show page_uri where page_uri_id = id
        //$pages =$this->MPages->getIdwithnone();
        $lilly_pagetable='omc_lilly_fairies_pages';
        $pages =$this->MPages->getIdwithnoneAll($lilly_pagetable);
        $data['pages']=$pages;
        $data['languages'] =$this->MLangs->getLangDropDownWithId();
        $data['header'] = $this->lang->line('kago_manage'). " Lilly Fairies ". $this->lang->line('kago_menu');
    ...
    function create(){
        ...
        if ($this->input->post('name')){
            ...
            $data['title'] = "Create Menu in Lilly Fairies";
        // need to show only english menus
	  	//$data['menus'] = $this->MMenus->getAllMenusDisplay();
            $lang_id ='0';
            $data['menus'] = $this->MMenus->getAllMenusDisplayByLang($lang_id,$this->table);
            ...
            $lillypagetable = 'omc_lilly_fairies_pages';
            $data['pages'] = $this->MPages->getIdwithnone($lillypagetable);
            // Set breadcrumb
            $this->bep_site->set_crumb($this->lang->line('kago_create'),$this->module.'/admin/create');

            $data['header'] = "Create Menu in Lilly Fairies";
  ...
  function edit($id=0){
	...
	// if segment 6 is 0 then it is English
        ...
        $data['title'] = "Lilly Fairies ".$this->lang->line('kago_edit')." ".$this->lang->line('kago_menu');
	...
	$data['page'] = $this->config->item('backendpro_template_admin') . "admin_menu_edit";
        ...
        $page_table='omc_lilly_fairies_pages';
        $data['pages'] = $this->MPages->getIdwithnoneLang($lang_id, $page_table);
		//$data['pages'] = $this->MPages->getAllPathwithnone();
        ...
        $data['menu'] = $this->MMenus->getMenu($id,$this->table);
        /**
        if (!count($data['menu'])){
         ...
         * 
         */
        $data['header'] = $this->lang->line('kago_edit');
    ...
    function deleteMenu($id){
        ...
        $menu = $this->MMenus->getMenu($id, $this->table);
        $parentid = $menu['parentid'];
        if(!$parentid==0){
            $orphans = $this->MMenus->checkMenuOrphans($id, $this->table);
            if (count($orphans)){
                ...
                $this->MMenus->deleteMenu($id,$this->table);
    ...
    function langcreate(){
        if ($this->input->post('name')){
            // info is filled out, so the followings
             $data = $this-> _fields();
             $this->MKaimonokago->addItem($this->table, $data);
            //$this->MMenus->addMenu();
            ...
            $checktrans = $this->MKaimonokago->checktrans($this->table, $id, $lang_id);
            if (count($checktrans)){
            ...
            $data['translanguages'] =$this->MLangs->getTransLang($this->table,$id);
