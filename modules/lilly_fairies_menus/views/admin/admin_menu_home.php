...
<div class="buttons">
	<a href="<?php print  site_url('lilly_fairies_menus/admin/create')?>">
 ...
 */
$multi_language=$this->preference->item('multi_language');
function generateRowsByLevel($navlist, &$output, $depth = 0,$pages,$languages, $multi_language) {
    ...
    if($multi_language==1){
        ...
        */
        $output .= "<td class=\"" . $depthClassMapping[$depth] . "\"><a href=\"". site_url(). '/lilly_fairies_menus/admin/edit/';
        ...
        $output .= anchor("kaimonokago/admin/changeStatus/".$module."_menus/".$list['id'],'<img src="'.base_url().'assets/icons/'.$active_icon.'.png"  alt="'.$active_icon.'" />', array('class' => $list['status']));
        $output .= "</td>\n";
        ...
        $output .= "<td align='center'>";
        $output .= anchor('lilly_fairies_menus/admin/edit/'. $list['id'],'<img src="'.base_url().'assets/icons/pencil.png"  alt="edit" />');
        ...
        // if ($list['status']=='inactive')
           // $output .= " | ";
            ...
            $output .= anchor("lilly_fairies_menus/admin/deleteMenu/". $list["id"],'<img src="'.base_url().'assets/icons/delete.png"  alt="delete" />', array("onclick"=>"return confirmSubmit('".$list['name']."')"));
        ...
        */
        ...
        // don't forget to add $module=$this->uri->segment(1); at the top of this page
        $active_icon = ($list['status']=='active'?'tick':'cross');
        $output .= anchor("kaimonokago/admin/changeStatus/lilly_fairies_menus/".$list['id'],'<img src="'.base_url().'assets/icons/'.$active_icon.'.png"  alt="'.$active_icon.'" />', array('class' => $list['status']));
        $output .= "</td>\n";
        //$output .= "<td align='center'>". $list['status']."</td>\n";
        $output .= "<td align='center'>". $list['parentid']."</td>\n";
        $output .= "<td class=\"" . $depthClassMapping[$depth] . "\" >". $list['order']."</td>\n";
        ...
        if (!empty($list['children'])) {
            generateRowsByLevel($list['children'], $output, $depth + 1,$pages,$languages, $multi_language);
        }
    }
}
...
if (count($navlist)){
    ...
    generateRowsByLevel($navlist, $output,$depth = 0,$pages,$languages, $multi_language);
    // close up the table
    $output .= "</tbody>\n</table></div>";
    // display table
    echo $output;
